...
export function manageUsers(state = Immutable.fromJS(initialState), action) {
  ...
  if (action.type == 'ACTION_SUCCESS_USER_PROFILE') {
    ...
    return state.set('user_profile_detail', action.payload.user_profile_detail).set('user_bank_detail', action.payload.user_bank_detail).set('username', action.username)
