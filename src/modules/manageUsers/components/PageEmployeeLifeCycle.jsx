...
class PageEmployeeLifeCycle extends Component {
  ...
  componentsWillMount (props) {
    ...
    this.props.requestEmployeLifeCycle(start_year, end_year);
  }
  ...
  render () {
    ...
    let EmpGraphData = _.map(EmpLife, (memberData, k) => {
      ...
      let dataStatus = _.map(memberData.Stats, (empData, j) => {
        ...
        // let joinee = _.map(empData.details, (newEmpData, m) => {
        //   countJoinees.push(newEmpData.count_joinees);
        //   countTerminations.push(newEmpData.count_terminations);
        //   return (
        //     <YAxis key={m} id="number">
    ...
    return (
      ...
      <div className="col-md-12 row">
        ...
        <div className="form-group col-md-4">
          <button type="button" style={{marginTop: '6%'}} onClick={(evt) => this.getByYearData()} className="btn btn-primary form-group">Get Details</button>
        </div>
      </div>
    </div>
    );
