...
export function applyPendingLeave (state = Immutable.fromJS(initialState), action) {
  if (action.type === 'ACTION_PENDING_LEAVE_SUCCESS') {
    return state.set('status_message', action.payload)
        ...
        .set('end_date', '');
  } else if (action.type === 'ACTION_PENDING_LEAVE_FAIL') {
    return state.set('status_message', action.payload)
        .set('count_working_days', '')
        .set('count_holidays', '')
        .set('count_weekends', '')
        .set('days_between_leaves', '')
        .set('end_date', '');
  } else if (action.type === 'ACTION_PENDING_LEAVE_ERROR') {
    return state.set('status_message', action.payload)
        .set('count_working_days', '')
        .set('count_holidays', '')
        .set('count_weekends', '')
